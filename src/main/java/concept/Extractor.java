...
public class Extractor {
	...
	public static String getConcept(ArrayList<String> seedList, String concept, int noOfResults, double overlapTolerance) {
		//freqMap.clear();
		//docMap.clear();
		int weight=1;
		ArrayList<WebPage> listpages = Web.getSearchResults(seedList, null, noOfResults, overlapTolerance); 
		...
		TreeMap<Double, String> concepScoreMap = new TreeMap<Double, String>();
		int count=0;
		while(count++<3){
			...
			double s = getConceptScore(candidateConcept,seedList,noOfResults);
			LogUtil.log.info("Trying out concept "+candidateConcept+" : got score :" +s);
			concepScoreMap.put(s,candidateConcept);
		}

		LogUtil.log.fine("in Extract.extractConcept() Freq Map : "+ freqMap);
		LogUtil.log.fine("in Extract.extractConcept() Doc Map : "+ docMap);
		LogUtil.log.fine("in Extract.extractConcept() Score concpet : "+ concepScoreMap);

		return concepScoreMap.lastEntry().getValue();
	}

	private static double getConceptScore(String candidateConcept, ArrayList<String> seedList, int noOfResults) {
		ArrayList<String> mockSeeds= new ArrayList<String>();
		mockSeeds.add(candidateConcept);
		ArrayList<String> concept = SearchProvider.bingUrls(mockSeeds, null, noOfResults,1);
	...
	public static boolean isValidConcept(ArrayList<String> seedList,
			int noOfResults, String candidateConcept) {
		double score=getConceptScore(candidateConcept, seedList,noOfResults);
